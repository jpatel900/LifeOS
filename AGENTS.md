...
6. **Status hygiene.** Update `docs/PROJECT_STATE.md` only when shipped behavior, status, or governance materially changes (replace, don't append) — and when you do, also triage the oldest undecided `docs/KNOWN_ISSUES.md` row. At every campaign close (or monthly), run a system review and file findings as KNOWN_ISSUES rows, pruning process weight that stopped earning its keep.
...
Merge authority, in one place: T0 docs-only PRs may auto-merge after CI per `.github/AGENT_AUTOMATION_POLICY.md` (tier definitions T0-T4 live there); an agent never merges its own PR (the self-approval classifier blocks it — the owner merges those; auto-merge armed by the T0 policy lane counts as the policy's merge, not the author's), with one ratified exception: per ADR 0008 move 2 (owner 2026-08-04), a `risk:low` non-T2+ agent PR may arm auto-merge after a 30-minute open window with the owner notified — effective once its enabling workflow ships; the Claude lane merges Codex-lane work per `docs/agent/LANES.md`; T2+ surfaces always get human review before merge. If another doc states a different merge rule, this paragraph wins — fix that doc.
