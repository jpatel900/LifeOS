...
/**
 * #689: seeds a raw capture (no parse — the exact state where the owner's
 * thought used to vanish from every triage surface).
 *
 * #703: `submitCaptureRaw` is gone because there is only one capture path
 * now, and it is this one — `submitCaptureText` persists the thought verbatim
 * and never parses. Unlike CaptureSeedBridge above, this bridge deliberately
 * does NOT auto-sort, so the capture stays unsorted and the #689 read-path
 * claim is still exactly what is under test.
 */
