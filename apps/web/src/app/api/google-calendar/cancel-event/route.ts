    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "Google Calendar isn't set up on LifeOS yet, so events can't be cancelled right now.",
      },
      { status: 503 },
    );
  }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Reconnect Google Calendar before cancelling events. Its saved access is no longer available.",
        },
        { status: 409 },
      );
    }
