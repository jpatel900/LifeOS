...
### 5. Skill-library compounding (TESTED 2026-07 — first result NEGATIVE for ambient loading)
...
**RESULT (2026-07-03, pre-registered A/B, this library):** on a 6-task planted-trap suite at Sonnet tier, with-library vs without showed **zero margin** (criterion was ≥+2 tasks). Two mechanisms explained all observations: (a) **ceiling** — the 2026 Sonnet baseline no longer commits four of the six classic failure modes on small single-session tasks (spec-vs-test conflicts, flaky root-cause, CI-vs-README, generated files); (b) **visibility ≠ invocation** — skills listed as available were not loaded/applied on the discriminating tasks (no agent in either condition wrote an unprompted NOT-verified line). Falsified: "ambient availability lifts short single-session tasks at Sonnet tier." NOT yet tested: explicit invocation (skill content placed in context), long-horizon/multi-session value (campaigns, chronicles — where the library's design center actually is), and real-repo tasks harder than the toy suite.
**RESULT 2 (2026-07-04, explicit-invocation arm):** loading the relevant SKILL.md into context (with ambient availability removed) ran against the same suite. Registered margin (≥+2 tasks) NOT met — post-audit the suite had exactly one valid discriminator left (four ceiling tasks; one grader voided as a fourth check bug). On that one discriminator (unprompted NOT-verified reporting), the effect was decisive: **ambient 0/5, without 0/5, explicit 5/5.** Status: "explicit loading changes behavior on the skill's target discipline" is a CANDIDATE with strong single-task evidence — not an accepted result. Practical default meanwhile: route skills by explicit loading, never assume availability does anything.
**Next falsifiable step (v4, if pursued):** ≥5 non-ceiling discriminating tasks (traps the current baseline still fails, verified by pilot), behavior-anchored graders only (artifact checks — keyword lists over free text are banned after four grader bugs), same ≥+2 margin, adversarial refutation on any positive.
