...
- [ ] If the diff adds live-DB RLS tests: they are inside apps/web/src/**tests**/phase4aRls.local.test.ts (any new \*.local.test.ts file does NOT run in CI — FAIL).
...
- [ ] No schema weakening: no dropped/renamed columns or tables, no loosened zod schemas, no removed/weakened tests or RLS policies (grep the diff for deletions in migrations, packages/schemas, **tests**).
