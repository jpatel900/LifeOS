...
| 8 | TODO/FIXME hotspots: `git grep -n -E "TODO|FIXME|HACK|XXX" -- . \| head -40` | Known-weak areas the team already knows about | Reporting known issues as discoveries |
| 9 | Generated-vs-source boundaries: `git grep -l -i -E "do not edit\|autogenerated\|auto-generated\|@generated" \| head -20` | Files you must never hand-edit | The edit-a-generated-file incident (see `agentic-change-control` non-negotiables) |

Steps 6, 8, 9 commands are identical in PowerShell (git carries its own tooling); only the `|` pipes behave the same by design.
...
- Re-verify the recon script still runs clean: `sh scripts/repo-recon.sh .` in any git repo (expect the seven `=====` sections, no errors).
