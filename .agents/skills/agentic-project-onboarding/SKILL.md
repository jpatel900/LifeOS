...
**Jargon:** _recon_ = read-only investigation before any change. _Entry file_ = the file an agent tool loads automatically (CLAUDE.md, AGENTS.md, .cursor/rules). _Churn_ = how often a file changes in git history.
...
- You catch yourself about to run a build/test command you _assumed_ rather than read.
...
| 2   | `README.md` + any manifest/vision doc                                                                        | What the project claims to be; documented commands (treat as _claims_ until step 4)                  | —                                                                                                                                         |
...
- [ ] You can state the build and test commands _as CI runs them_, and you have run the test suite (or the relevant slice) once, green, before touching anything — a pre-existing red baseline discovered _after_ your edits is indistinguishable from damage you caused.
