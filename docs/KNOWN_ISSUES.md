...
| 9 | ~~CI ran the full e2e (~6 min) + migrations-rls (~3 min) jobs on docs-only PRs~~ | low | 2026-06-23 | **resolved 2026-06-23** — added a `changes` detection job; `e2e`/`migrations-rls` now gate on `if: needs.changes.outputs.heavy == 'true'` (matches `apps/`, `packages/`, `supabase/`, `scripts/`, workflows, dep manifests), `validate-monorepo` stays unconditional. Used `if`-gating rather than workflow-level `paths:` so skipped jobs still satisfy required checks instead of deadlocking |
