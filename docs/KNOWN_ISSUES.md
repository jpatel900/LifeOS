...
| 9 | CI `pull_request` trigger runs the full e2e (~6 min) + migrations-rls (~3 min) jobs on docs-only PRs (observed on #222) | low | 2026-06-23 | pending — add `paths` filters (`apps/web/**`, `supabase/**`) so pure-docs PRs skip the heavy jobs; saves ~9 CI min/PR (NFR-001 / AGENTS §14). Keep `validate-monorepo` unfiltered |
