...
- **Status:** Fenced two ways. (1) The kick now treats UNCONFIRMED PR existence as failure: because the sandbox cannot reach github.com, every task must post `PATCH-PASTE DELIVERY` comments in the SAME run unless it positively fetched its PR URL — never wait to be asked. (2) Recovery for already-stranded work goes through the OWNER opening the original task session ("View task →" link in its completion comment) and asking it to print or post the patch — never a fresh mention. If the original session has also lost its environment, the work is unrecoverable: re-implement in the other lane rather than re-kick.
- **Date:** 2026-07-05
