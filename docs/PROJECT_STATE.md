...
- Persisted planning, execution, review, and health remain local-only. Phase 7C adds OAuth consent and metadata-only connection state, but there is still no Google token persistence, free/busy query, calendar event insert, review/health narrative AI, autonomous scheduling, background job, advanced analytics, or conflict auto-solver.
...
2. Start Phase 7D only after explicit scope approval; keep it limited to free/busy conflict checking and local conflict display, with no Google Calendar event insert.
...
- Phase 7B does not store Google OAuth tokens. `google_calendar_connections` is metadata-only until an explicit encrypted token storage PR lands, and `external_write_events` should store summaries only, never raw Google payloads or secrets.
- Phase 7C still does not store Google OAuth tokens. The OAuth callback uses a short-lived sealed HttpOnly cookie carrying the initiating Supabase access token only long enough to finish the RLS-authenticated metadata write, then clears it immediately.
