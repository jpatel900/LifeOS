...
## 0. Two historical contradictions this mechanism caught (worked proof, not current status)

These were found by reading four sibling plans against `origin/main` in 2026-07. They remain load-bearing worked examples of **drift as propagation lag**, but they are not current-status claims. C-0A was subsequently resolved; current ADRs, REQUIREMENTS, and implemented registries/guards own the outcome.

### C-0A — FR-number collision (FUNCTIONAL drift; resolved 2026-07-05)

- The collision was real: **FR-022 = WIP Enforcement**, while the early task-map draft also used FR-022.
- ADR 0004 and the current coherence registry/REQUIREMENTS resolved it: WIP remains FR-022 and task-map v1 is FR-031.
- The task-map contract now uses FR-031, matching the shipped feature. This section remains a worked example of the failure G-FUNC-1 catches, not an unresolved task.
...
3. **Extend-don't-parallel.** The registry's _narrative_ home is the HARMONY MATRIX section that `plan-daily-driver-floor.md` already pioneered — that matrix becomes a short, human-readable section appended to REQUIREMENTS (or its own ADR), and the JSON is its machine-checkable projection. One source of truth (JSON), one human view (the matrix + generated map), one guard (the test). No third bureaucracy.
...
**Extends:** the daily-driver-floor HARMONY MATRIX (its per-conflict-with-resolution format is the proven shape), REQUIREMENTS.md FRs (the authority for _what_ a feature is), and ENGINEERING_INVARIANTS.md (INV-1..7, NS-INV-1..9, UX-INV-1..6 are the invariant vocabulary an entry references).
  ...
  "features": [
    {
      ...
      "interacts_with": [
        {
          "fr": "FR-027",
          "kind": "X", // C compatible / S synergy / X conflict
          "note": "shares capture surface; online-parse => FR-026 containment, save-raw/offline => FR-027 queue; never both waiting.",
          "resolution_ref": "plan-daily-driver-floor.md §3 C5/#368 row",
        }, // X REQUIRES a resolution ref
      ],
    },
  ],
  "policies": [
    {
      "id": "wip_enforcement.v1",
      "fr": "FR-022",
...
- **From task-map plan:** FR-031 task-map with `policy_ids: ["task_map.v1"]` and its interaction notes. C-0A documents the historical number correction.
...
- **G-FUNC-1 — FR referential integrity.** Every `features[].fr` and every `interacts_with[].fr` matches a unique `### FR-NNN` heading in REQUIREMENTS.md; no two features share an `fr`; no `FR-NNN` heading is duplicated. _(This is the guard that catches C-0A.)_
...
**Sequencing safety (§5 rule, stated once): each guard must be green on main the day it lands.** C-0A historically required fix-then-guard and is now resolved by ADR 0004/current registry plus task-map FR-031. The worked lesson remains: reconcile a conflict in the same change before enabling its guard. A guard that reddens main blocks the sequential one-slice relay (NS-INV-6) and is itself a coherence failure.
...
### (a) Interaction-pattern registry — R3 _[gate-only]_
...
Named patterns (seed set — the vocabulary of _how the operator acts_, capped at ~8):
...
### (b) Keyboard-map registry — R4 _[implementable]_
...
### (c) Color/shape semantics — R5 _[implementable]_
...
### (d) Copy-voice rules — R6 _[gate-only]_
...
### (f) Surface budget per moment — R8 _[gate-only]_
...
### (g) Map-view question — R9 _[gate-only, MANDATORY field]_
...
The pass rides three existing surfaces. Nothing here is a new gate; each is a _specialization_ of a step that already exists.
...
### 3.4 When the full-corpus coherence audit runs (proposal incorporated into the ritual)

Run a **full-corpus coherence audit** when an owner-ratified capability wave is contracted after its structural prerequisites and any applicable capability-specific evidence gates are satisfied. Regenerate the map (§4), run all coherence guards over the whole registry (not just the diff), and walk the HARMONY MATRIX for any `X` edge whose `resolution_ref` has gone stale. Per-slice merges do the _incremental_ check (§3.2); the contract session does the _whole-corpus_ check. No blanket previous-stage usage gate, standing cron, or per-session burden.
...
Historical packet plan: packet = one PR; N=new / E=edit; each independently CI-green; **every guard green on main the day it lands or fix-then-guard in the same PR**. C-0A/CO-1a is completed; preserve it as the worked reconciliation example.
...
| **CO-1** Referential-integrity guard        | `coherenceRegistry.test.ts`: G-FUNC-1..5.                                                                                                                            | N `apps/web/src/__tests__/coherenceRegistry.test.ts`                                                                    | sonnet (contracted; mirror `docRegistry.test.ts`) | Historical fix-then-guard prerequisite C-0A is resolved.  | after CO-0 in original plan.                                   |
| **CO-1a** C-0A reconciliation — COMPLETED   | Resolved FR-number collision: WIP FR-022; task-map FR-031 in REQUIREMENTS/current registry and task-map artifact.                                                    | Historical docs/registry reconciliation                                                                                 | opus                                              | made CO-1 green                                           | completed 2026-07-05.                                          |
...
- Do NOT registry every component, every CSS class, every string. The registries capture _shared grammar that means something_ (patterns, keys, the two color axes, voice, motion budget) — not the full UI inventory.
...
2. **Placement argued:** a machine-readable `docs/coherence-registry.json` + `coherenceRegistry.test.ts` + a _generated_ human map — a faithful imitation of `doc-registry.json` + `docRegistry.test.ts`; NOT prose-in-REQUIREMENTS (unparseable) and NOT a new narrative markdown doc (rots the doc-registry surface).
3. **It caught real drift:** C-0A's historical FR-number collision was resolved 2026-07-05 through ADR 0004/current registry and task-map FR-031; it remains a worked G-FUNC-1 example. C-0B records the token triple-naming example (`--area-accent`/`--amb-*` vs `--st-*` vs `--state-*`).
...
9. **Packets (CO-0..CO-7):** historical implementation plan; C-0A/CO-1a is completed. Its durable lesson is fix-then-guard so no red guard blocks the relay.
