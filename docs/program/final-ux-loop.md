...
- **2026-08-20 — S6 IN FLIGHT (shell close-out, PR #880):** S4 Health port (#846) and S5 Areas port (#851) both merged since the 08-06 entry below — the "In flight: S4 Health port" note there is historical, not current. C2-S6 (this PR) is the campaign's final slice: `/calendar`, `/review`, `/health`, `/areas` retire to flag-gated redirect shims in one atomic change (the `#590` rollback stays fully functional), closes the Criterion 2/3 URL-truth gaps (moment joins the URL contract; `?moment=` + `?sheet=` compose), and re-anchors the gated cockpit e2e specs. C2's three Target Card 2 criteria (Structure, IA 5.0→9, zero margin — 2/3 closed = 7.67 < 9) all close in this one PR by contract design; see PR #880 for the evidence-anchored report.
