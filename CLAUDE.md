...
Implementation truth lives in `docs/`: `REQUIREMENTS.md`, `ARCHITECTURE.md`, `DATA_MODEL.md`, `UX_FLOWS.md`, `SECURITY_PRIVACY.md`, `TEST_PLAN.md`, with ADRs in `docs/adr/` amending architecture. `docs/PROJECT_STATE.md` is the cross-agent handoff file — worth reading when you need current status, and worth updating after changes that alter shipped behavior or governance so Codex agents picking up later aren't misled.
